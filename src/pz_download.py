...
def trigger_export(project_id, token):
    ...
    for i in range(60):
        ...
        data = r.json()
        if data.get("status") == 1:  # completed
            return data
...
def download_artifact(project_id, token, output_dir):
    ...
    with zipfile.ZipFile(buf) as zf:
        # PZ exports have a "utf8/" prefix typically
        for info in zf.infolist():
            if info.is_dir():
                continue
            # Normalize path: strip leading "utf8/", "raw/", and version prefix
            name = info.filename
            parts = name.split("/")
            if parts[0] in ("utf8", "raw"):
                parts = parts[1:]
            # Detect and strip version prefix like "0.9.159/" if present
            import re as _re
            if parts and _re.match(r'^\d+\.\d+\.\d+$', parts[0]):
                if not hasattr(download_artifact, '_detected_version'):
                    download_artifact._detected_version = parts[0]
                parts = parts[1:]
            rel = "/".join(parts)
...
def main():
    ...
    # Detect version from PZ directory structure
    detected_ver = getattr(download_artifact, '_detected_version', '')
    if detected_ver:
        print(f"Detected PZ version: {detected_ver}")
    ...
    if gh:
        with open(gh, "a") as f:
            ...
            if detected_ver:
                f.write(f"pz_version={detected_ver}\n")
